...
/**
 * Conta quantas tarefas já existem na lista para uma determinada unidade.
 * Busca todas as tarefas (com paginação) e filtra pelo campo Unidade no Node.js
 * — mais confiável do que depender do filtro via query param do ClickUp.
 */
async function getCountByUnidade(unidade) {
  const labelId = LABEL_IDS[unidade];
  if (!labelId) {
    console.warn(`[WARN] Label UUID não encontrado para "${unidade}" — usando contagem geral.`);
    return await getCountGeral();
  }
  ...
  while (true) {
    ...
    // Filtra tarefas que têm o label da unidade no campo customizado
    for (const task of tasks) {
      const campo = (task.custom_fields || []).find(f => f.id === FIELD_IDS.unidade);
      if (!campo) continue;
      const valores = Array.isArray(campo.value) ? campo.value : [];
      // O ClickUp retorna labels como array de objetos { id, label } ou array de strings
      const match = valores.some(v => (v?.id ?? v) === labelId);
      if (match) total++;
    }
...
/**
 * Fallback: conta todas as tarefas da lista (sem filtro de unidade).
 */
async function getCountGeral() {
  let total = 0;
  let page  = 0;
  while (true) {
    const { data } = await clickup.get(`/list/${CU_LIST_ID}/task`, {
...
app.post("/api/solicitacao", requireApiKey, async (req, res) => {
  ...
  // ── Buscar contagem por unidade para numeração sequencial por região ──
  let baseCount = 0;
  try {
    baseCount = await getCountByUnidade(dados.unidade);
    console.log(`[INFO] Tarefas encontradas para "${dados.unidade}": ${baseCount} → próxima será ${baseCount + 1}`);
  } catch (err) {
    console.warn("[WARN] Não foi possível obter contagem por unidade:", err.message);
  }

  // ── Processar extras em paralelo ──
  const resultados = await Promise.allSettled(
    dados.extras.map(async (extra, i) => {
      const numero      = baseCount + i + 1;
